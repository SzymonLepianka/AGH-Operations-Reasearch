...
def run_example(game_path, expected_pure, expected_mixed):
    ...
    print(mixed_eq)
    print("----------")
    assert len(pure_eqs) == len(
        expected_pure), "Your algorithm found incorrect amount of pure equilibriums!"
    for i, eq in enumerate(pure_eqs):
        assert eq.equals_enough(expected_pure[i])
    assert mixed_eq.equals_enough(
        expected_mixed), "Your algorithm found an incorrect pure equilibrium!"
